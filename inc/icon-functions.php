 ...
 */
function hotel_wp_get_fo( $args = array() ) {
	// Make sure $args are an array.
	if ( empty( $args ) ) {
		return __( 'Please define default parameters in the form of an array.', 'hotel-wp-lite' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return __( 'Please define an fontawesome icon filename.', 'hotel-wp-lite' );
	}
 ...
 */
function hotel_wp_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	if ( 'top' === $args->theme_location ) {
		foreach ( $item->classes as $value ) {
			if ( 'menu-item-has-children' === $value || 'page_item_has_children' === $value ) {
				$title = $title . hotel_wp_get_fo( array( 'icon' => 'angle-down' ) );
			}
		}
	}
...
add_filter( 'nav_menu_item_title', 'hotel_wp_dropdown_icon_to_menu_link', 10, 4 );
